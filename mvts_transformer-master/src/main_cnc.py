...
from datasets.cncdata import CNCDataset

folder = '../../OfficialDataset'
...
def main(config):
    ...
    train_data = CNCDataset(os.path.join(folder, f"{signal}_tf_train"))
    val_data = CNCDataset(os.path.join(folder, f"{signal}_tf_val"))
    test_data = CNCDataset(os.path.join(folder, f"{signal}_tf_test"))
    ...
    for epoch in tqdm(range(start_epoch + 1, config["epochs"] + 1), desc='Training Epoch', leave=False):
        ...
        #print("HHHHHHHHHH", aggr_metrics_test['loss'])
...
if __name__ == '__main__':
    ...
    #main(config)
    config['signal'] = 'first'
    log_file = os.path.join(DIRECTORY, f"{config['signal']}_loggings")

    for i in range(8):
        results = main(config)
        print(f"result for attempt {i} {results}")
        with open(log_file, 'a') as df:
            df.write(",".join(str(ele) for ele in results))
            df.write('\n')
        
    print(f"DONE {config['signal']}!!!!")

    config['signal'] = 'second'
    log_file = os.path.join(DIRECTORY, f"{config['signal']}_loggings")
